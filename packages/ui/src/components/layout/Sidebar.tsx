  ...
  return (
    ...
    >
      ...
      <ScrollArea className="flex min-h-0 flex-1 flex-col w-full">
        <div className={cn('py-2 flex flex-col', collapsed ? 'items-center gap-1' : 'space-y-0.5')}>
          {primaryNav.map(({ icon: Icon, label, href }) => {
            ...
            const itemClass = cn(
              ...
              collapsed ? 'size-11 justify-center' : 'min-h-11 gap-2.5 px-3 py-2 w-full',
            );
      ...
      <div className={cn('py-2 flex flex-col', collapsed ? 'items-center' : '')}>
        <Tooltip>
          <TooltipTrigger
            className={cn(
